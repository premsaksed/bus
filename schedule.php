						...
						<table class="table table-striped table-bordered" id="schedule-field">
							...
							<thead>
								...
								<th class="text-center">#</th>
									...
									<th class="text-center">รถบัส</th>
									<th class="text-center">สถานี</th>
									<th class="text-center">การออกเดินทาง</th>
									<th class="text-center">จุดหมาย</th>
...
<script>
						...
						resp = JSON.parse(resp)
							if(Object.keys(resp).length > 0){
								...
								Object.keys(resp).map(k=>{
									...
									tr.append('<td><center><button class="btn btn-sm btn-primary edit_schedule mr-2" data-id="'+resp[k].id+'">Edit</button><button class="btn btn-sm btn-danger remove_schedule" data-id="'+resp[k].id+'">ลบ</button></center></td>')
									}else{
										tr.append('<td><center><button class="btn btn-sm btn-primary mr-2 text-white book_now" data-id="'+resp[k].id+'"><strong>จองเลย</strong></button></center></td>')
									}
	...
	function manage(){
		...
		uni_modal('แก้ไข','manage_schedule.php?id='+$(this).attr('data-id'))
